# Inventory App v5 — AGENTS.md

> This file is the single source of truth for Codex sessions on this project.
...
Returns do **not** restore items directly into the original FIFO lots. The original order keeps its FIFO COGS and seller-paid shipping cost, while refunds reduce revenue by `refunds.amount`. Each selected return line defaults to restocking the full return quantity, but the user can uncheck restock or lower `restock_quantity` when only part of the returned quantity is sellable. Restocked goods create a new inventory intake lot with `quantity = restock_quantity`, `cost_price = restock_unit_cost`, and `shipping_cost = restock_shipping_cost` (cost fields default to 0). The return form labels these as optional resale cost: returned item value per unit and extra restock/refurb cost. They are hidden behind a details section because most returns should stay at 0 to avoid confusing the original sale COGS with the new returned-goods lot. If the user forgets to restock during return creation, Return Detail provides an after-the-fact restock action for items that do not yet have a linked returned-goods lot. Restocking changes stock only; it does not change revenue, refunds, COGS, seller shipping, or gross profit.

Returned-goods lots can later be written off from Return Detail if inspection shows items are damaged, unsellable, lost, or otherwise unavailable. The guardrail is lot-level: write-off quantity must be greater than 0 and cannot exceed the lot's current `remaining_quantity`; already-resold returned units cannot be written off.
...
See `SCHEMA_CHANGELOG.md` for the full history of applied migrations.
...
**Mobile tables**:
- Any user-facing `<table class="table">` should also include `mobile-card-table`. At phone widths, CSS hides the header and renders each row as a labeled card.
- `templates/base.html` runs `applyMobileTableLabels()` on page load and after HTMX swaps, copying header text into missing `data-label` attributes.
- `templates/base.html` derives the top `.app-title` from each template's `{% block title %}` with the trailing app name removed. Override `{% block header_title %}` only when a page needs a custom short header.
- Dashboard/list filter tabs wrap into touch-sized buttons on mobile instead of using horizontal scrolling.
- Inventory uses product-level summary cards with search/status filters first; lot-level cost/date/write-off rows are collapsed under per-product details so mobile users do not have to scroll through every lot for every product.
- Return item selection and after-the-fact return restock use custom cards instead of table-to-card conversion so common fields stay compact and rare returned-goods cost fields are hidden behind "Optional resale cost".

**Mobile modals**:
- Modal form partials use `.modal-body` for the scrollable field area and `.modal-footer` for always-reachable actions.
- Use `.modal-field-grid` for two-column form rows and `.modal-code-row` for input + generate-button rows; both collapse to one column on phone widths.
...
- **Mobile table/card layout** - phone-width list/detail/report tables use `mobile-card-table`, auto-filled row labels from table headers, wrapped filter tabs, product-summary inventory cards, return-item cards, and scroll-contained modal forms to avoid horizontal side-scrolling
- **List browsing controls** — Products, Categories, Customers, Inventory, Orders, Returns, and Refunds have searchable/sortable GET controls, selectable rows per page, visible result counts, and compact pagination; sort keys are validated through `list_utils.py`
...
- **FIFO date warning** — intake form warns when `intake_date` is >30 days in the past (FIFO ordering concern)
- **Processing order accounting** — moving Draft → Processing reserves stock through FIFO and counts revenue/COGS/debt; Draft remains editable/deletable with no stock or accounting effect
- **FIFO order completion** — direct Draft → Completed deducts from oldest lot first; Processing → Completed allocates any still-unreserved backorder quantity without duplicating prior allocations
- **Order reopen/cancel** — precisely restores exact inventory lots and removes active-sale accounting
- **Draft order delete** — Draft orders can be deleted because they have no allocations, revenue, COGS, or debt effect
- **Order form stock hints** — product selector shows current stock; highlights zero/low stock in amber/red
- **Return quantity validation** — server-side check: returned qty ≤ original ordered qty
...
- **Dashboard** — Net Revenue, COGS (FIFO), Seller Shipping, Gross Profit, Margin; period filter
  - Outstanding collections widget (unpaid processing/completed orders total)
  - Separate negative-stock counter (backorders) vs low-stock counter
  - Top 5 products and top 5 customers for the selected period
  - Payment status column in Recent Orders table
- **COGS fallback fix** — checks `COUNT(allocations) > 0` not `SUM > 0` (prevents silent override by `products.cost_price`)
- **Reports** — COGS uses FIFO allocations throughout (not `products.cost_price`)
  - Profit report: fixed COGS in overall P&L, by-month, and by-product views
  - Inventory report: stock value uses `inventory.cost_price` per lot (not `products.cost_price`)
  - Customer Debt Aging report: 0–7 / 8–30 / 31–90 / 90+ day buckets with per-order detail
- **Export** — CSV and Excel (XLSX, requires openpyxl) for Products, Customers, Orders, Inventory; "Export All" single XLSX with 4 sheets
- **DB migrations** — safe column additions via `_migrate_add_column()` on every startup
- **Schema versioning** — `SCHEMA_VERSION` constant in `db.py`; embedded in backup filenames
