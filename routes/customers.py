...
def customers_list():
    ...
    try:
        ...
        count_params = []
        if search:
            count_query += ' AND (c.name LIKE ? OR c.customer_code LIKE ? OR c.email LIKE ?)'
        ...
        return render_template('customers.html', customers=customers, page=page, total_pages=total_pages, search=search)
    finally:
        db.close()
