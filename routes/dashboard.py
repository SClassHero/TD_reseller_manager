...
def _calc_cogs(cursor, start_date_str, end_date_str):
    """
    Calculate COGS for completed orders in the date range.
    ...
    Fallback path: only used for orders that have NO allocation records at all
    (i.e. legacy orders created before the FIFO allocation system existed).
    Uses products.cost_price, or average intake cost if that is also missing.

    IMPORTANT: the check is 'do any allocations exist?' (COUNT > 0), NOT
    'is the sum > 0?'.  The old 'if cogs > 0' check was a bug that silently
    fell through to products.cost_price whenever intake costs were recorded
    as 0 (free items, blank cost fields, etc.).
    """
    try:
        cursor.execute('''
            SELECT
                COUNT(oa.id)                                                    AS alloc_count,
        ...
        row = cursor.fetchone()
        if row and row['alloc_count'] > 0:
            # Allocations exist → trust them even if cost is 0 (genuinely free items)
            return row['alloc_cogs'] or 0.0
    except Exception:
        pass

    # Fallback: no FIFO allocations found (legacy orders).
    # Use products.cost_price, or average inventory intake cost.
    cursor.execute('''
        ...
        WHERE o.order_status = 'completed'
          AND o.order_date >= ? AND o.order_date <= ?
    ''', (start_date_str, end_date_str))
    return cursor.fetchone()['total_cogs'] or 0.0
...
def get_revenue_and_profit_data(db, start_date_str, end_date_str):
    ...
    for completed orders in the date range.

    Accounting model (standard inventory management):
      Gross Revenue  = SUM(total_amount) from completed orders
      Total Refunds  = SUM(refunds.amount) linked to completed orders in range
    ...
    # Total Refunds issued against completed orders in this period
    # (refund_date falls within the period, linked to a completed order)
    cursor.execute('''
    ...
    try:
        ...
        seller_shipping = 0.0  # column may not exist in old DBs
...
def dashboard():
    ...
    try:
        ...
        cursor.execute('''
            ...
            WHERE order_status IN ('draft', 'processing')
        ''')
        ...
        cursor.execute('''
            SELECT o.id, o.order_code, o.order_date, c.name as customer_name, o.total_amount, o.order_status
            FROM orders o
